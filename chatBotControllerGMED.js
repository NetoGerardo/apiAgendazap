...
const gestorMed = require('./integracao/GestorMed.js');
...
async function passarClienteProximaEtapa(wpp_client, cliente_funil, mensagem_atual, json_resposta, resposta_para_enviar, passar_para_proxima_etapa, encerrar_funil, proxima_tag, mensagem_recebida) {
    ...
    if (passar_para_proxima_etapa) {
        if (mensagem_atual.save_data && mensagem_atual.save_data != "") {
            ...
            if (mensagem_atual.save_data == "whatsapp") {
                meta_dados[mensagem_atual.save_data] = cliente_funil.numero;
            } else {
...
async function getMessageFullfilment(mensagem_funil, cliente_funil, callback) {
    ...
    if (mensagem_funil.tag == "agendamento-consulta-#mg1") {
        ...
        getMedicosEspecialidades(cliente_funil, (response) => {
            salvarRespostasEsperadas(cliente_funil, response.respostas_esperadas)
    ...
    gestorMed.listarEspecialidades("econ").then(especialidades => {
        ...
        for (let i = 0; i < especialidades.length; i++) {
            ...
            let option = {
                ...
                valor: especialidades[i].IdEspecialidade,
            ...
            mensagem = mensagem + "*" + (i + 1) + "* - " + especialidades[i].Especialidade + "\n"
        }
...
async function getMedicosEspecialidades(cliente_funil, callback) {

    getUpdatedClient(cliente_funil, (cliente) => {

        let dados = JSON.parse(cliente.meta_dados);

        gestorMed.listarMedicos("econ", dados.tipo_consulta).then(resposta => {

            let response = {
                mensagem: resposta.mensagem,
                respostas_esperadas: resposta.options
            }
...
async function getClinicas(cliente_funil, callback) {
    ...
    getUpdatedClient(cliente_funil, (cliente) => {
        ...
        let dadosGMED = extrairDadosGMED(dados.medico_data);
        ...
        let option2 = {
            resposta_esperada: 2,
            valor: 2,
            proxima_tag: "agendamento-consulta-#mg1"
        }
...
function sendConfirmationMessage(cliente) {
    ...
    let dadosGMED = extrairDadosGMED(cliente.medico_data);

    gestorMed.buscarHorario(dadosGMED.data, dadosGMED.medico).then(response => {

        mensagem = mensagem +
            "\n*Paciente:* " + cliente.nome_paciente +
